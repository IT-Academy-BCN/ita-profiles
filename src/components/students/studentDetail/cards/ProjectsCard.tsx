        ...
        {!isLoadingProjects && (
          <div ref={carouselRef} className="flex gap-3 overflow-x-hidden">
            {projectsData.map((project) => (
              <div key={project.id} className="flex flex-col gap-1 rounded-xl border border-gray-3 px-5 py-3.5">
                ...
                <div className="flex items-center justify-between pt-3">
                  <div className="text-sm rounded-lg border border-black-3 px-2 py-1 font-semibold">
                    {project.tags
                      .slice(0, 2)
                      .map((tag, index) => (
                        <span key={tag.id}> {tag.name} {index !== 1 && ' · '} </span>
                      ))
                    }
                  </div>
                    <a href={project.project_url} type="button" className="h-8 rounded-lg border border-black-3">
                      <img src={ArrowRightProjects} alt="right arrow button" className="h-full"/>
                    </a>
                </div>
              </div>
            ))}
